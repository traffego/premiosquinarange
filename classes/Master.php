...
error_reporting(1);
		...
		if ($insert) {
			...
			if ($use_manual_numbers) {
				...
				$cotas_rua_check = $this->conn->query('SELECT cotas_rua_inicio, cotas_rua_fim, cotas_rua_liberadas, qty_numbers FROM product_list WHERE id = \'' . $product_id . '\'')->fetch_assoc();
				if (!empty($cotas_rua_check['cotas_rua_inicio']) && !empty($cotas_rua_check['cotas_rua_fim'])) {
					...
					$liberados = json_decode($cotas_rua_check['cotas_rua_liberadas'] ?? '[]', true) ?: [];
					$numeros_bloqueados = [];
					foreach ($numbers as $num) {
						$num_int = (int)$num;
						if ($num_int >= $rua_inicio && $num_int <= $rua_fim && !in_array($num_int, $liberados)) {
							$numeros_bloqueados[] = $num;
						}
					}
					if (!empty($numeros_bloqueados)) {
						$bloqueados_str = implode(',', $numeros_bloqueados);
						$resp['status'] = 'failed';
						$resp['error'] = (count($numeros_bloqueados) > 1 ? 'Os números ' . $bloqueados_str . ' estão reservados para venda presencial.' : 'O número ' . $bloqueados_str . ' está reservado para venda presencial.');
...
$cotas_rua_data = $this->conn->query('SELECT cotas_rua_inicio, cotas_rua_fim, cotas_rua_liberadas FROM product_list WHERE id = \'' . $product_id . '\'')->fetch_assoc();
$numeros_rua = [];
if (!empty($cotas_rua_data['cotas_rua_inicio']) && !empty($cotas_rua_data['cotas_rua_fim'])) {
    $liberados_auto = json_decode($cotas_rua_data['cotas_rua_liberadas'] ?? '[]', true) ?: [];
    for ($r = (int)$cotas_rua_data['cotas_rua_inicio']; $r <= (int)$cotas_rua_data['cotas_rua_fim']; $r++) {
        if (!in_array($r, $liberados_auto)) {
            $numeros_rua[] = str_pad($r, max((int) $globos, strlen($qty_numbers)), "0", STR_PAD_LEFT);
        }
    }
}

while (count($numeris) < $qty_numbers) {
    ...
    if (!in_array($randomNumber, $numeris) && !in_array($randomNumber, $numeros_ja_vendidos) && !in_array($randomNumber, $numeros_rua)) {
        $numeris[] = $randomNumber;
    }
...
$update = $this->conn->query(
    'UPDATE `order_list` SET `order_numbers` = \'' .
        ...
        '\''
			...
			if (($this->settings->info('mercadopago') == 1) &&  $total_amount > 0) {
				mercadopago_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration);
			}
			if (($this->settings->info('gerencianet') == 1) &&  $total_amount > 0) {
	...
	if (($this->settings->info('pagstar') == 1) &&  $total_amount > 0) {
				pagstar_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration);
			}
		...
		// order_email($this->settings->info('email_order'), '[' . $this->settings->info('name') . '] - Confirmação de pedido', $oid); // ENVIO DE EMAIL DESATIVADO
...
// order_email($this->settings->info('email_purchase'), '[' . $this->settings->info('name') . '] - Pagamento aprovado', $id);
			...
			if (($status_order == '2') && $status == '1') {
				...
				// CHAMADA DE EMAIL COMENTADA - NÃO UTILIZADA NO DIA A DIA
// order_email($this->settings->info('email_order'), '[' . $this->settings->info('name') . '] - Confirmao de pedido', $id);
			}
	...
	public function check_order_status2()
	{
		extract($_POST);
			...
			while ($row = $qry->fetch_assoc()) {
				...
				if (($expirationTime < $currentDateTime) && 0 < $orderExpiration && $row['status'] != '3') {
					if ($payment_method == 'MercadoPago') {
						...
						if (check_order_pagstar($order_id, $id_mp) == 'failed') {
							$query = $this->conn->query('UPDATE order_list SET status = 3 WHERE id = \'' . $order_id . '\'');
							$query2 = $this->conn->query('UPDATE product_list SET pending_numbers = pending_numbers - \'' . $quantity . '\' WHERE id = \'' . $product_id . '\'');
							revert_product($product_id);
						}
	 ...
	 if (0 < $qry->num_rows) {
            while ($row = $qry->fetch_assoc()) {
                ...
                if (
					...
					$qry = $this->conn->query("UPDATE  `order_list` SET `status` = '3' WHERE `order_token` = '".$order_token."'");
	...
	$resp['ex']= $expirationTime < $currentDateTime;
	$resp['pm']= $payment_method;
	...
	public function export_raffle_contacts2()
	{
		...
		$qry = $this->conn->query('SELECT o.*, CONCAT(c.firstname, \' \', c.lastname) as customer, p.type_of_draw, c.phone, o.id, o.order_numbers' . "\r\n\t\t\t" . 'FROM `order_list` o' . "\r\n\t\t\t" . 'INNER JOIN customer_list c ON o.customer_id = c.id' . "\r\n\t\t\t" . 'INNER JOIN product_list p ON o.product_id = p.id' . "\r\n\t\t\t" . $where . "\r\n\t\t\t" . 'ORDER BY ABS(UNIX_TIMESTAMP(o.date_created)) DESC');
		$lista = [];
	...
	public function contact_send_email()
	{
		...
		if (!$_settings->info('smtp_host')) {
			...
			try {
				...
				echo 'No foi possvel enviar a mensagem. Mailer Error: ' . $mail->ErrorInfo;
				$resp['status'] = 'failed';
			}
		}
	...
	public function recover_password()
	{
		...
		if (0 < $qry->num_rows) {
			...
			if ($update_pass) {
				if (!$_settings->info('smtp_host')) {
					$message .= 'Olá, vimos que voc solicitou uma recuperação de senha, aqui estão os dados da sua nova senha:\\n\\n';
					...
					require_once '../libs/phpmailer/src/SMTP.php';
					$mail = new PHPMailer\PHPMailer\PHPMailer(true);
					$message .= 'Ol, vimos que você solicitou uma recuperação de senha, aqui esto os dados da sua nova senha:<br><br>';
	...
	public function search_raffle_winner()
	{
		...
		if ($type_of_draw == 3) {
			$bichos = ['00' => 'Avestruz', '01' => 'Águia', '02' => 'Burro', '03' => 'Borboleta', '04' => 'Cachorro', '05' => 'Cabra', '06' => 'Carneiro', '07' => 'Camelo', '08' => 'Cobra', '09' => 'Coelho', 10 => 'Cavalo', 11 => 'Elefante', 12 => 'Galo', 13 => 'Gato', 14 => 'Jacar', 15 => 'Leão', 16 => 'Macaco', 17 => 'Porco', 18 => 'Pavo', 19 => 'Peru', 20 => 'Touro', 21 => 'Tigre', 22 => 'Urso', 23 => 'Veado', 24 => 'Vaca'];
		}
	...
	public function verify_orders_mp()
	{
		...
		if (0 < $orders->num_rows) {
			...
			echo '<hr>Fim da verificaão de pedidos.';
		}
		else {
	...
	public function correct_duplicates()
	{
		...
		if ($id) {
			...
			$execution_time = ($time_end - $time_start) / 60;
			echo 'Processo realizado com sucesso.<br>';
			echo 'Tempo de durao: ' . $execution_time;
		}
	}
