		...
		if (flock($lock, LOCK_EX)) {
			...
			if ($insert) {
				...
				if ($use_manual_numbers) {
					...
					// Excluir cotas de rua da geração automática
					...
					if (!empty($cotas_rua_data['cotas_rua_inicio']) && !empty($cotas_rua_data['cotas_rua_fim'])) {
						...
						$numeros_ja_vendidos = array_merge($numeros_ja_vendidos, $numeros_rua);
					}
				...
				if (($this->settings->info('nextpay') == 1) &&  $total_amount > 0) {
					nextpay_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration);
				}
				if (($this->settings->info('openpix') == 1) &&  $total_amount > 0) {
