...
class Main extends DBConnection
{
    ...
    public function save_product()
    {
        ...
        $cotas_rua_inicio = (isset($_POST['cotas_rua_inicio']) && $_POST['cotas_rua_inicio'] !== '') ? (int)$_POST['cotas_rua_inicio'] : 'NULL';
        $cotas_rua_fim = (isset($_POST['cotas_rua_fim']) && $_POST['cotas_rua_fim'] !== '') ? (int)$_POST['cotas_rua_fim'] : 'NULL';
        ...
        $first_purchase_free_qty = isset($_POST["first_purchase_free_qty"]) ? (int)$_POST["first_purchase_free_qty"] : 0;
        ...
        if (empty($id)) {
            ...
            // Verificar se o sorteio já tem pedidos — se sim, não permitir alterar cotas de rua
            $check_orders = $this->conn->query('SELECT pending_numbers, paid_numbers FROM product_list WHERE id = ' . $id)->fetch_assoc();
            if (((int)$check_orders['pending_numbers'] + (int)$check_orders['paid_numbers']) == 0) {
                // Remove trailing semicolon and WHERE clause, append cotas_rua, then re-add WHERE
                $sql = rtrim($sql, ';');
                $sql = preg_replace('/WHERE `id` = ' . $id . '$/', ', `cotas_rua_inicio` = ' . $cotas_rua_inicio . ', `cotas_rua_fim` = ' . $cotas_rua_fim . ' WHERE `id` = ' . $id . ';', $sql);
            }
        }
    ...
    public function place_order()
    {
        ...
        if (flock($lock, LOCK_EX)) {
            ...
            $query =
                'SELECT discount_qty, enable_discount, discount_amount, enable_cumulative_discount, enable_sale, sale_qty, sale_price, status, qty_numbers, pending_numbers, paid_numbers, date_of_draw, first_purchase_enabled, first_purchase_free_qty, price FROM product_list WHERE id = \'' .
                $product_id .
                '\'';
            ...
            if ($result && 0 < $result->num_rows) {
                ...
                $first_purchase_enabled = $row["first_purchase_enabled"];
                $first_purchase_free_qty = $row["first_purchase_free_qty"];
                $unit_price_base = $row["price"];
            }
            ...
            // Lógica Promoção Primeira Compra
            if ($first_purchase_enabled == 1 && $first_purchase_free_qty > 0 && $quantity > 0) {
                 // Verificar pedidos anteriores pagos
                 $check_previous = $this->conn->query("SELECT id FROM order_list WHERE customer_id = '$customer_id' AND product_id = '$product_id' AND status = '2' LIMIT 1");
                 
                 if ($check_previous->num_rows == 0) {
                     // É a primeira compra paga. Calcular desconto.
                     // Preço efetivo por cota
                     $effective_price = ($total_amount > 0) ? ($total_amount / $quantity) : 0;
                     
                     // Quantidade a descontar (limitada à quantidade do pedido)
                     $qty_to_discount = min($quantity, $first_purchase_free_qty);
                     
                     $discount_value = $qty_to_discount * $effective_price;
                     ...
                     if ($discount_value > 0) {
                         ...
                         // Se order_discount_amount estiver vazio ou for string, inicializar
                         if (!is_numeric($order_discount_amount)) $order_discount_amount = 0;
                         $order_discount_amount += $discount_value;
                     }
                 }
            }

            // Verificar se total é zero APÓS todos os descontos (incluindo promo primeira compra)
            $pay_status = 1;
            if ($total_amount == 0) {
                $pay_status = 2;
                $this->conn->query(
            ...
            $insert = $this->conn->query(
                'INSERT INTO `order_list` (`code`, `customer_id`, `product_name`, `quantity`, `status`, `total_amount`, `order_token`, `order_numbers`, `product_id`, `order_expiration`, `discount_amount`,`roleta`, `date_created`) VALUES (\'' .
                    $code .
                    '\', \'' .
                    $customer_id .
                    '\', \'' .
            ...
            if ($insert) {
                ...
                if ($use_manual_numbers) {
                    ...
                    $orders = $this->conn->query(
                        "SELECT order_list.order_numbers, product_list.cotas_premiadas, product_list.status_auto_cota, product_list.paid_numbers, product_list.tipo_auto_cota, product_list.qty_numbers
                        FROM order_list
                ...
                if (($this->settings->info('pagstar') == 1) &&  $total_amount > 0) {
                    pagstar_generate_pix(
                        $oid,
                        $total_amount,
                        $customer_name,
                        $customer_email,
                ...
                if (!empty($data)) {
                    ...
                    $resp["status"] = "success";
                    $resp["memory"] = get_memory_usage() / 1024 / 1024 . "MB";
                }
            } else {
                $resp["status"] = "failed";
                $resp["error"] = $this->conn->error;
    ...
    public function att_roleta()
    {
        ...
        $roletaserach = $this->conn->query("SELECT roleta FROM `order_list` where order_token = '$order_token'");
        $roletaserach = $roletaserach->fetch_assoc();

        $roletaserach['roleta'] -= 1;
        $qry = $this->conn->query('UPDATE order_list SET roleta = \'' . $roletaserach['roleta'] . '\' WHERE order_token = \'' . $order_token . '\'');
    ...
    public function check_payment_status()
    {
        ...
        if (0 < $qry->num_rows) {
            while ($row = $qry->fetch_assoc()) {
                ...
                if ($payment_method == "Pagstar") {
                    check_order_pagstar($order_id, $id_mp);
                }
            }
        }
    ...
    public function load_numbers()
    {
        ...
        if ($status == 1) {
            $firstnames = [];
            $stmt_plist = $this->conn->prepare(
                "SELECT qty_numbers, pending_numbers, paid_numbers, cotas_rua_inicio, cotas_rua_fim, cotas_rua_liberadas FROM `product_list` WHERE id = ?"
            );
            ...
            // Excluir cotas de rua dos números livres (exceto os liberados)
            ...
            $cotas_rua_liberadas = json_decode($product['cotas_rua_liberadas'] ?? '[]', true) ?: [];
            $cotas_rua_qty = 0;
            if ($cotas_rua_inicio > 0 && $cotas_rua_fim > 0 && $cotas_rua_fim >= $cotas_rua_inicio) {
                $cotas_rua_qty = ($cotas_rua_fim - $cotas_rua_inicio) + 1 - count($cotas_rua_liberadas);
	...
	WHERE o.product_id = '{$id}'");
            ...
            // Marcar cotas de rua como usadas (exceto as liberadas)
            if ($cotas_rua_inicio > 0 && $cotas_rua_fim > 0) {
                for ($r = $cotas_rua_inicio; $r <= $cotas_rua_fim; $r++) {
                    if (!in_array($r, $cotas_rua_liberadas)) {
...
");
    ...
    public function duplicate_product()
    {
        ...
        if ($qry->num_rows > 0) {
            ...
            $check_slug = $this->conn->query('SELECT * FROM `product_list` where `slug` LIKE \'' . $slug . '%\'')->num_rows;
            ...
            $values = implode("', '", array_map([$this->conn, 'real_escape_string'], array_values($product)));
            ...
            foreach ($cotas_premiadas_sold as $cota) {
                ...
                if ($cota != '') {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        ...
                        //  echo '<div class="rounded p-1 w-100 d-flex justify-content-center align-items-center">';
                        //  echo '    <div class="numero rounded-pill d-flex justify-content-center align-items-center btn text-start '.$btnTheme.' btn-sm p-0 text-nowrap font-xss">';
                        //  echo '        <span class="px-2" style="min-width: 100px">🚀 ' . $cota . '</span>';
                        //  echo '       <span class="d-flex justify-content-start align-items-center valor rounded-pill py-0 px-2  btn btn-warning" style="min-width: 130px"><i class="bi bi-circle-fill me-3" style="font-size: 5px;"></i>' . $customer_name . '</span>';
                        //  echo '    </div>';
                        //  echo '    <div class="status col ms-3 '.$textTheme.'" style="font-weight: bold">';
                        //  echo $prize . ' 🏆';
                        //  echo '   </div>';
                        //  echo '</div>';
                        ...
                        //  echo '            <span  class="new_gradient_anime --md ' . $tipo . '" style="min-width:37px !important; border-radius: 10px; height:37px  ; padding:14px 10px;font-weight:500; margin-right:4px; font-size:14px">' . $cota . '</span>';
                        //  echo '          <span class="prize" style=" font-weight:500; margin-right:8px; ">' . $prize . '</span>';
                        //  echo '       </div>';
                        //  echo '         <span style="text-wrap:nowrap; font-size:14px">' . $customer_name . ' 🏆</span>';
                        //  echo '    </div>';

                        //  echo '   </div>';
                        //  echo '<div class="hr"></div>';

                        // echo '<div class=" sc-3f9a15f1-7 reservada p-1" style=" background: #00307A; border-radius: 10px; margin-bottom: 6px; ">  ';
                        // echo '    <div style="justify-content:space-between ;display:flex; align-items:center ; ">  ';
                        // echo '       <div style="display:flex; align-items:center ;   justify-content: space-between; width: 100%; padding: 0 4px 0 0;">       ';
                        // echo '            <span class="wd-5 new_gradient_anime --md Premiada" style="min-width:75px !important; border-radius: 10px; height:25px  ; padding:14px 10px;font-weight:500; margin-right:4px; font-size:14px;background-color:#fff; color:black">' . $cota . '</span>  ';
            ...
            foreach ($cotas_premiadas_available as $cota) {
                ...
                if ($cota != '') {
                    ...
                    if ($produto['roleta'] || $produto['box']) {
                        ...
                        echo '    <div class="numero rounded-pill d-flex justify-content-center align-items-center btn text-start ' . $btnTheme . ' btn-sm p-0 text-nowrap font-xss">';
                        echo '        <span class="px-2" style="min-width: 150px">🚀 ' . $cota . ' vale ' . $prize . '</span>';
                        echo '       <span class="d-flex justify-content-start align-items-center valor rounded-pill py-0 px-2  btn btn-success" style="min-width: 130px"><i class="bi bi-circle-fill me-3" style="font-size: 5px;"></i>Disponível</span>';
                        echo '    </div>';
                        echo '</div>';
                    } else {
                        //  echo '<div class=" rounded p-1 w-100 d-flex justify-content-center align-items-center">';
                        ...
                        // echo '<div class=" sc-3f9a15f1-7 disponivel p-1" style="background: #dddddd;border-radius: 10px; margin-bottom: 6px;">';
                        // echo '    <div style="justify-content:space-between ;display:flex; align-items:center ; ">';
                        // echo '        <div style="display:flex; align-items:center  ;   justify-content: space-between; width: 100%; padding: 0 4px 0 0;">';
                        // echo '             <span class="wd-6 new_gradient_anime --md Premiada" style="min-width:75px !important; border-radius: 10px; height:25px  ;   padding:14px 10px; font-weight:500; margin-right:4px; font-size:14px;background-color:#81889e !important">'.$cota.'</span>';
                        // echo '            <span class="prize" style=" font-weight:500; margin-right:8px;color:black ">'.$prize.'</span>';
                        // echo '            <span style="text-wrap:nowrap; font-size:14px;font-weight:600; color:black">Disponível </span>';
                        // echo '         </div>';
                        // echo '    </div>';
                        // echo '</div>';
                        ...
                        // echo '          <span class="prize" style=" font-weight:500; margin-right:8px; ">' . $prize . '</span>';
                        // echo '       </div>';
                        // echo '         <span style="text-wrap:nowrap; font-size:14px">Disponível </span>';
                        // echo '    </div>';
    ...
    /**
     * Gera cotas premiadas automaticamente para um sorteio
     * Evita números já vendidos e já cadastrados como premiados
     */
    public function generate_cotas_premiadas()
    {
        $resp = array();
        
        // Validar parâmetros
        if (!isset($_POST['sorteio_id']) || empty($_POST['sorteio_id'])) {
            $resp['status'] = 'failed';
            $resp['error'] = 'ID do sorteio não informado';
            return json_encode($resp);
        }
        
        if (!isset($_POST['quantidade']) || empty($_POST['quantidade'])) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Quantidade não informada';
            return json_encode($resp);
        }
        
        if (!isset($_POST['premio']) || empty($_POST['premio'])) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Prêmio não informado';
            return json_encode($resp);
        }
        
        $sorteio_id = $this->conn->real_escape_string($_POST['sorteio_id']);
        $quantidade = intval($_POST['quantidade']);
        $premio = $this->conn->real_escape_string(trim($_POST['premio']));
        
        if ($quantidade < 1) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Quantidade deve ser maior que zero';
            return json_encode($resp);
        }
        
        // Buscar informações do sorteio
        $query = "SELECT qty_numbers, paid_numbers, pending_numbers, cotas_premiadas, cotas_premiadas_premios 
                  FROM product_list 
                  WHERE id = '$sorteio_id' AND status = 1";
        
        $result = $this->conn->query($query);
        
        if ($result->num_rows == 0) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Sorteio não encontrado ou inativo';
            return json_encode($resp);
        }
        
        $sorteio = $result->fetch_assoc();
        $qty_numbers = intval($sorteio['qty_numbers']);
        $paid_numbers = intval($sorteio['paid_numbers']);
        $pending_numbers = intval($sorteio['pending_numbers']);
        $cotas_premiadas_existentes = $sorteio['cotas_premiadas'];
        $cotas_premiadas_premios_existentes = $sorteio['cotas_premiadas_premios'];
        
        // Calcular números disponíveis
        $numeros_disponiveis = $qty_numbers - $paid_numbers - $pending_numbers;
        
        if ($quantidade > $numeros_disponiveis) {
            $resp['status'] = 'failed';
            $resp['error'] = "Quantidade solicitada ($quantidade) é maior que os números disponíveis ($numeros_disponiveis)";
            return json_encode($resp);
        }
        
        // Array para armazenar números ocupados (vendidos, reservados e premiados)
        $numeros_ocupados = array();
        
        // 1. Buscar números já vendidos ou reservados
        $query_vendidos = "SELECT order_numbers FROM order_list WHERE product_id = '$sorteio_id' AND status <> 3";
        ...
        while ($row = $result_vendidos->fetch_assoc()) {
            if (!empty($row['order_numbers'])) {
                ...
                foreach ($numbers as $num) {
                    $num = trim($num);
                    if ($num !== '') {
                        $numeros_ocupados[$num] = true;
                    }
                }
            }
        }
        
        // 2. Adicionar cotas premiadas já cadastradas aos números ocupados
        if (!empty($cotas_premiadas_existentes)) {
            $cotas_array = explode(',', $cotas_premiadas_existentes);
            foreach ($cotas_array as $cota) {
                $cota = trim($cota);
                if ($cota !== '') {
                    $numeros_ocupados[$cota] = true;
                }
            }
        }
        
        // 3. Gerar números aleatórios disponíveis
        $cotas_geradas = array();
        $tentativas = 0;
        $max_tentativas = $qty_numbers * 10; // Evitar loop infinito
        
        // Calcular o tamanho correto para os números (padding com zeros)
        $tamanho_numero = strlen((string)($qty_numbers - 1));
        
        while (count($cotas_geradas) < $quantidade && $tentativas < $max_tentativas) {
            $tentativas++;
            
            // Gerar número aleatório
            $numero_aleatorio = mt_rand(0, $qty_numbers - 1);
            $numero_formatado = str_pad($numero_aleatorio, $tamanho_numero, '0', STR_PAD_LEFT);
            
            // Verificar se o número está disponível
            if (!isset($numeros_ocupados[$numero_formatado]) && !in_array($numero_formatado, $cotas_geradas)) {
                $cotas_geradas[] = $numero_formatado;
            }
        }
        
        // Verificar se conseguiu gerar a quantidade solicitada
        if (count($cotas_geradas) < $quantidade) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Não foi possível gerar a quantidade solicitada de cotas. Apenas ' . count($cotas_geradas) . ' cotas disponíveis foram encontradas.';
            return json_encode($resp);
        }
        ...
        // Adicionar cotas existentes
        if (!empty($cotas_premiadas_existentes)) {
            $cotas_existentes_array = explode(',', $cotas_premiadas_existentes);
            $novas_cotas = array_merge($novas_cotas, $cotas_existentes_array);
        }
        
        if (!empty($cotas_premiadas_premios_existentes)) {
            $premios_existentes_array = explode(',', $cotas_premiadas_premios_existentes);
            $novos_premios = array_merge($novos_premios, $premios_existentes_array);
        }
        
        // Adicionar novas cotas geradas
        foreach ($cotas_geradas as $cota) {
            $novas_cotas[] = $cota;
            // Formato: NUMERO:PREMIO:tipo
            $novos_premios[] = $cota . ':' . $premio . ':premiada';
        }
        
        // Converter arrays em strings
        $cotas_premiadas_final = implode(',', $novas_cotas);
        $cotas_premiadas_premios_final = implode(',', $novos_premios);
        ...
        $stmt->bind_param('sssi', $cotas_premiadas_final, $cotas_premiadas_premios_final, $cotas_premiadas_final, $sorteio_id);
        
        if ($stmt->execute()) {
            $resp['status'] = 'success';
            $resp['msg'] = "✅ {$quantidade} cotas premiadas geradas com sucesso!";
            $resp['cotas_geradas'] = $cotas_geradas;
            $resp['premio'] = $premio;
        } else {
            $resp['status'] = 'failed';
            $resp['error'] = 'Erro ao salvar cotas premiadas no banco de dados: ' . $this->conn->error;
        }
    ...
    /**
     * Busca informações detalhadas sobre cotas premiadas de um sorteio
     * Retorna quantas estão vendidas e quantas disponíveis
     */
    public function get_cotas_info()
    {
        $resp = array();
        ...
        // Buscar cotas premiadas do sorteio
        $query = "SELECT cotas_premiadas, cotas_premiadas_premios FROM product_list WHERE id = '$sorteio_id'";
        $result = $this->conn->query($query);
        
        if ($result->num_rows == 0) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Sorteio não encontrado';
            return json_encode($resp);
        }
        ...
        // Separar as cotas
        $cotas_array = explode(',', $cotas_premiadas);
        $total = count($cotas_array);
        
        // Verificar quais estão vendidas
        $vendidas = 0;
        foreach ($cotas_array as $cota) {
            $cota = trim($cota);
            if (empty($cota)) continue;
            
            $stmt = $this->conn->prepare('SELECT id FROM order_list WHERE FIND_IN_SET(?, order_numbers) AND product_id = ? AND status = 2');
            $stmt->bind_param('si', $cota, $sorteio_id);
            $stmt->execute();
        ...
        $resp['status'] = 'success';
        $resp['total'] = $total;
        $resp['disponiveis'] = $disponiveis;
        $resp['vendidas'] = $vendidas;
        
        return json_encode($resp);
    }

    /**
     * Exclui cotas premiadas em batch (lote)
     * Só remove cotas que NÃO foram vendidas
     */
    public function delete_cotas_premiadas_batch()
    {
        $resp = array();
        ...
        $sorteio = $result->fetch_assoc();
        $cotas_premiadas = $sorteio['cotas_premiadas'];
        $cotas_premiadas_premios = $sorteio['cotas_premiadas_premios'];
        
        if (empty($cotas_premiadas)) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Este sorteio não possui cotas premiadas cadastradas';
            return json_encode($resp);
        }
        
        // Separar as cotas
        $cotas_array = explode(',', $cotas_premiadas);
        $premios_array = !empty($cotas_premiadas_premios) ? explode(',', $cotas_premiadas_premios) : array();
        
        // Reverter arrays para pegar as mais recentes primeiro (LIFO - Last In First Out)
        $cotas_array = array_reverse($cotas_array);
        
        // Criar mapa de prêmios por cota
        $premios_map = array();
        foreach ($premios_array as $premio_str) {
            if (empty($premio_str)) continue;
            $parts = explode(':', $premio_str, 2);
            if (count($parts) >= 2) {
                $numero = trim($parts[0]);
                $premios_map[$numero] = $premio_str;
            }
        }
        
        // Identificar cotas disponíveis (não vendidas) e vendidas
        $cotas_disponiveis = array();
        $cotas_vendidas_count = 0;
        $cotas_excluidas = array();
        
        foreach ($cotas_array as $cota) {
            $cota = trim($cota);
            if (empty($cota)) continue;
            
            // Verificar se a cota foi vendida
            $stmt = $this->conn->prepare('SELECT id FROM order_list WHERE FIND_IN_SET(?, order_numbers) AND product_id = ? AND status = 2');
            $stmt->bind_param('si', $cota, $sorteio_id);
            $stmt->execute();
            $result_check = $stmt->get_result();
            ...
            if ($result_check->num_rows > 0) {
                // Cota vendida - não pode ser excluída
                $cotas_vendidas_count++;
            } else {
                // Cota disponível - pode ser excluída
                $cotas_disponiveis[] = $cota;
            }
            $stmt->close();
        }
        
        // Verificar se há cotas disponíveis suficientes
        if (count($cotas_disponiveis) == 0) {
            $resp['status'] = 'failed';
            $resp['error'] = 'Não há cotas disponíveis para exclusão. Todas as cotas premiadas já foram vendidas.';
            return json_encode($resp);
        }
        
        if ($quantidade > count($cotas_disponiveis)) {
            $resp['status'] = 'failed';
            $resp['error'] = "Quantidade solicitada ($quantidade) é maior que as cotas disponíveis para exclusão (" . count($cotas_disponiveis) . ")";
            return json_encode($resp);
        }
        
        // Selecionar as cotas a serem excluídas (as mais recentes disponíveis)
        $cotas_para_excluir = array_slice($cotas_disponiveis, 0, $quantidade);
        $cotas_excluidas = $cotas_para_excluir;
        
        // Criar novas listas sem as cotas excluídas
        $novas_cotas = array();
        $novos_premios = array();
        ...
        foreach ($cotas_array as $cota) {
            ...
            // Se a cota não está na lista de exclusão, mantém
            if (!in_array($cota, $cotas_para_excluir)) {
                $novas_cotas[] = $cota;
                
                // Manter o prêmio associado
                if (isset($premios_map[$cota])) {
                    $novos_premios[] = $premios_map[$cota];
                }
            }
        }
        ...
        if ($stmt->execute()) {
            $resp['status'] = 'success';
            $resp['msg'] = count($cotas_excluidas) . " cota(s) premiada(s) excluída(s) com sucesso!";
            $resp['cotas_excluidas'] = $cotas_excluidas;
            $resp['cotas_protegidas'] = $cotas_vendidas_count;
        } else {
            $resp['status'] = 'failed';
            $resp['error'] = 'Erro ao atualizar o banco de dados: ' . $this->conn->error;
        }
...
switch ($action) {
    ...
    case "load_cotas":
        ...
        break;
    case "generate_cotas_premiadas":
        echo $Main->generate_cotas_premiadas();
        break;
    case "get_cotas_info":
        echo $Main->get_cotas_info();
        break;
    case "delete_cotas_premiadas_batch":
        echo $Main->delete_cotas_premiadas_batch();
        break;
    default:
        break;
    public function get_cotas_rua_status()
    {
        $product_id = isset($_GET['product_id']) ? (int)$_GET['product_id'] : 0;
        $offset = isset($_GET['offset']) ? (int)$_GET['offset'] : 0;
        $limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 100;
        $filter = isset($_GET['filter']) ? $_GET['filter'] : 'all';
        $range_from = isset($_GET['range_from']) ? (int)$_GET['range_from'] : 0;
        $range_to = isset($_GET['range_to']) ? (int)$_GET['range_to'] : 0;

        if (!$product_id) {
            echo json_encode(['status' => 'error', 'error' => 'Product ID required']);
            return;
        }

        $product = $this->conn->query("SELECT cotas_rua_inicio, cotas_rua_fim, cotas_rua_liberadas FROM product_list WHERE id = '{$product_id}'")->fetch_assoc();
        if (!$product || empty($product['cotas_rua_inicio']) || empty($product['cotas_rua_fim'])) {
            echo json_encode(['status' => 'error', 'error' => 'Product not found or cotas de rua not configured']);
            return;
        }

        $inicio = (int)$product['cotas_rua_inicio'];
        $fim = (int)$product['cotas_rua_fim'];
        $liberadas = json_decode($product['cotas_rua_liberadas'] ?? '[]', true) ?: [];
        $total_range = $fim - $inicio + 1;
        $total_liberadas = count($liberadas);
        $total_bloqueadas = $total_range - $total_liberadas;

        $from = ($range_from > 0) ? max($range_from, $inicio) : $inicio;
        $to = ($range_to > 0) ? min($range_to, $fim) : $fim;

        $numeros = [];
        $count = 0;
        for ($i = $from; $i <= $to; $i++) {
            $is_liberado = in_array($i, $liberadas);
            if ($filter === 'blocked' && $is_liberado) continue;
            if ($filter === 'released' && !$is_liberado) continue;
            if ($count >= $offset && $count < $offset + $limit) {
                $numeros[] = ['numero' => $i, 'status' => $is_liberado ? 'liberado' : 'bloqueado'];
            }
            $count++;
        }
    ...
    public function toggle_cotas_rua_liberadas()
    {
        $product_id = isset($_POST['product_id']) ? (int)$_POST['product_id'] : 0;
        $numeros = isset($_POST['numeros']) ? $_POST['numeros'] : [];
        $action = isset($_POST['action']) ? $_POST['action'] : 'toggle';

        if (!$product_id || empty($numeros)) {
            echo json_encode(['status' => 'error', 'error' => 'Product ID and numeros required']);
            return;
        }
        ...
        foreach ($numeros as $num) {
            $num = (int)$num;
            if ($num < $inicio || $num > $fim) continue;
            $idx = array_search($num, $liberadas);
            if ($action === 'liberar' || ($action === 'toggle' && $idx === false)) {
                if ($idx === false) {
                    $liberadas[] = $num;
                    $alterados[] = ['numero' => $num, 'novo_status' => 'liberado'];
                }
            } elseif ($action === 'bloquear' || ($action === 'toggle' && $idx !== false)) {
                if ($idx !== false) {
                    array_splice($liberadas, $idx, 1);
                    $alterados[] = ['numero' => $num, 'novo_status' => 'bloqueado'];
                }
            }
        }

        sort($liberadas);
        $json = json_encode(array_values($liberadas));
        $this->conn->query("UPDATE product_list SET cotas_rua_liberadas = '" . $this->conn->real_escape_string($json) . "' WHERE id = '{$product_id}'");
