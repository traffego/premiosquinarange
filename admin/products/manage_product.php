...
<?= isset($description) ? $description : '' ?>
                ...
                <div id="tab8" class="tabcontent text-gray-700 dark:text-gray-400" style="display:none">
                    ...
                    <?php if (isset($id) && isset($cotas_rua_inicio) && !empty($cotas_rua_inicio) && isset($cotas_rua_fim) && !empty($cotas_rua_fim)) { ?>
                    <div id="cotas-rua-manager" class="mt-4" style="border: 1px solid #4a5568; border-radius: 8px; padding: 16px; background: #1a202c;">
                        <h4 style="color: #e2e8f0; margin-bottom: 12px; font-size: 16px;">🔧 Gerenciar Números Reservados</h4>
                        <div id="cotas-rua-resumo" style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
                            <span style="background: #e53e3e; color: #fff; padding: 4px 12px; border-radius: 16px; font-size: 13px;">🔴 Bloqueados: <strong id="rua-total-bloqueadas">-</strong></span>
                            <span style="background: #38a169; color: #fff; padding: 4px 12px; border-radius: 16px; font-size: 13px;">🟢 Liberados: <strong id="rua-total-liberadas">-</strong></span>
                            <span style="background: #4a5568; color: #e2e8f0; padding: 4px 12px; border-radius: 16px; font-size: 13px;">Total: <strong id="rua-total-range">-</strong></span>
                        </div>
                        <div style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap; align-items: center;">
                            <select id="rua-filter" style="padding: 6px 10px; border-radius: 6px; border: 1px solid #4a5568; background: #2d3748; color: #e2e8f0; font-size: 13px;"><option value="all">Todos</option><option value="blocked">Só bloqueados</option><option value="released">Só liberados</option></select>
                            <input type="number" id="rua-range-from" placeholder="De" style="width: 80px; padding: 6px 8px; border-radius: 6px; border: 1px solid #4a5568; background: #2d3748; color: #e2e8f0; font-size: 13px;">
                            <input type="number" id="rua-range-to" placeholder="Até" style="width: 80px; padding: 6px 8px; border-radius: 6px; border: 1px solid #4a5568; background: #2d3748; color: #e2e8f0; font-size: 13px;">
                            <button type="button" id="rua-apply-filter" style="padding: 6px 12px; border-radius: 6px; border: none; background: #805ad5; color: #fff; font-size: 13px; cursor: pointer;">Filtrar</button>
                            <button type="button" id="rua-select-mode" style="padding: 6px 12px; border-radius: 6px; border: 1px solid #805ad5; background: transparent; color: #805ad5; font-size: 13px; cursor: pointer;">☑ Seleção</button>
                        </div>
                        <div id="rua-batch-actions" style="display: none; gap: 8px; margin-bottom: 12px; flex-wrap: wrap; align-items: center;">
                            <button type="button" id="rua-select-all" style="padding: 5px 10px; border-radius: 6px; border: 1px solid #4a5568; background: #2d3748; color: #e2e8f0; font-size: 12px; cursor: pointer;">Selecionar todos visíveis</button>
                            <button type="button" id="rua-deselect-all" style="padding: 5px 10px; border-radius: 6px; border: 1px solid #4a5568; background: #2d3748; color: #e2e8f0; font-size: 12px; cursor: pointer;">Limpar seleção</button>
                            <button type="button" id="rua-batch-liberar" style="padding: 5px 10px; border-radius: 6px; border: none; background: #38a169; color: #fff; font-size: 12px; cursor: pointer;">🟢 Liberar selecionados</button>
                            <button type="button" id="rua-batch-bloquear" style="padding: 5px 10px; border-radius: 6px; border: none; background: #e53e3e; color: #fff; font-size: 12px; cursor: pointer;">🔴 Bloquear selecionados</button>
                            <span id="rua-selected-count" style="color: #a0aec0; font-size: 12px;">0 selecionados</span>
                        </div>
                        <div id="rua-grid" style="display: flex; flex-wrap: wrap; gap: 6px; max-height: 400px; overflow-y: auto; padding: 4px;">
                            <div id="rua-loading" style="width: 100%; text-align: center; padding: 20px; color: #a0aec0;">Carregando...</div>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Tabs restauradas pelo DOMContentLoaded acima (vanilla JS)
        //End tabs
        ...
        // ===== Cotas de Rua Manager =====
        <?php if (isset($id) && isset($cotas_rua_inicio) && !empty($cotas_rua_inicio) && isset($cotas_rua_fim) && !empty($cotas_rua_fim)) { ?>
        (function() {
            var productId = <?= $id ?>;
            var ruaOffset = 0, ruaLimit = 100, ruaHasMore = true, ruaLoading = false, ruaSelectMode = false, ruaSelected = {};
            var ruaGrid = document.getElementById("rua-grid");
            function ruaUpdateSummary(d) { $("#rua-total-bloqueadas").text(d.total_bloqueadas); $("#rua-total-liberadas").text(d.total_liberadas); $("#rua-total-range").text(d.total_range); }
            function ruaUpdateSelectedCount() { $("#rua-selected-count").text(Object.keys(ruaSelected).length + " selecionados"); }
            function ruaCreateBtn(item) {
                var b = document.createElement("button"); b.type = "button"; b.className = "rua-num-btn"; b.dataset.numero = item.numero; b.dataset.status = item.status; b.textContent = item.numero;
                b.style.cssText = "min-width:52px;padding:6px 4px;border-radius:6px;border:2px solid transparent;font-size:12px;font-weight:600;cursor:pointer;transition:all .15s;";
                b.style.background = item.status === "liberado" ? "#38a169" : "#e53e3e"; b.style.color = "#fff";
                b.addEventListener("click", function() {
                    if (ruaSelectMode) { if (ruaSelected[item.numero]) { delete ruaSelected[item.numero]; b.style.borderColor = "transparent"; b.style.boxShadow = "none"; } else { ruaSelected[item.numero] = item.status; b.style.borderColor = "#805ad5"; b.style.boxShadow = "0 0 0 2px #805ad5"; } ruaUpdateSelectedCount(); }
                    else { ruaToggle([item.numero]); }
                });
                return b;
            }
            function ruaLoadNumbers(reset) {
                if (ruaLoading) return; if (!reset && !ruaHasMore) return;
                if (reset) { ruaOffset = 0; ruaHasMore = true; ruaGrid.innerHTML = "<div style=\"width:100%;text-align:center;padding:20px;color:#a0aec0\">Carregando...</div>"; }
                ruaLoading = true;
                var url = _base_url_ + "class/Main.php?action=get_cotas_rua_status&product_id=" + productId + "&offset=" + ruaOffset + "&limit=" + ruaLimit + "&filter=" + $("#rua-filter").val();
                var f = $("#rua-range-from").val(), t = $("#rua-range-to").val(); if (f) url += "&range_from=" + f; if (t) url += "&range_to=" + t;
                $.getJSON(url, function(d) {
                    if (d.status === "success") { if (ruaOffset === 0) ruaGrid.innerHTML = ""; ruaUpdateSummary(d); d.numeros.forEach(function(i) { ruaGrid.appendChild(ruaCreateBtn(i)); }); ruaHasMore = d.has_more; ruaOffset += d.numeros.length; if (!ruaHasMore && d.numeros.length === 0 && ruaOffset === 0) ruaGrid.innerHTML = "<div style=\"width:100%;text-align:center;padding:20px;color:#a0aec0\">Nenhum n&uacute;mero encontrado</div>"; }
                    else { if (ruaOffset === 0) ruaGrid.innerHTML = "<div style=\"width:100%;text-align:center;padding:20px;color:#e53e3e\">" + (d.error || "Erro") + "</div>"; }
                    ruaLoading = false;
                }).fail(function() { ruaLoading = false; });
            }
            function ruaToggle(nums, act) {
                act = act || "toggle";
                $.post(_base_url_ + "class/Main.php?action=toggle_cotas_rua_liberadas", { product_id: productId, "numeros[]": nums, action: act }, function(d) {
                    var r = typeof d === "string" ? JSON.parse(d) : d;
                    if (r.status === "success") { $("#rua-total-bloqueadas").text(r.total_bloqueadas); $("#rua-total-liberadas").text(r.total_liberadas); r.alterados.forEach(function(a) { var btn = ruaGrid.querySelector("[data-numero=\"" + a.numero + "\"]"); if (btn) { btn.dataset.status = a.novo_status; btn.style.background = a.novo_status === "liberado" ? "#38a169" : "#e53e3e"; } }); }
                    else { alert(r.error || "Erro"); }
                }, "json");
            }
            if (ruaGrid) ruaGrid.addEventListener("scroll", function() { if (ruaGrid.scrollTop + ruaGrid.clientHeight >= ruaGrid.scrollHeight - 50) ruaLoadNumbers(false); });
            $("#rua-apply-filter").click(function() { ruaSelected = {}; ruaUpdateSelectedCount(); ruaLoadNumbers(true); });
            $("#rua-select-mode").click(function() { ruaSelectMode = !ruaSelectMode; if (ruaSelectMode) { $(this).css({background:"#805ad5",color:"#fff"}); $("#rua-batch-actions").css("display","flex"); } else { $(this).css({background:"transparent",color:"#805ad5"}); $("#rua-batch-actions").hide(); ruaSelected = {}; ruaUpdateSelectedCount(); ruaGrid.querySelectorAll(".rua-num-btn").forEach(function(b){b.style.borderColor="transparent";b.style.boxShadow="none";}); } });
            $("#rua-select-all").click(function() { ruaGrid.querySelectorAll(".rua-num-btn").forEach(function(b){ruaSelected[b.dataset.numero]=b.dataset.status;b.style.borderColor="#805ad5";b.style.boxShadow="0 0 0 2px #805ad5";}); ruaUpdateSelectedCount(); });
            $("#rua-deselect-all").click(function() { ruaSelected={}; ruaGrid.querySelectorAll(".rua-num-btn").forEach(function(b){b.style.borderColor="transparent";b.style.boxShadow="none";}); ruaUpdateSelectedCount(); });
            $("#rua-batch-liberar").click(function() { var n=Object.keys(ruaSelected); if(!n.length) return alert("Selecione pelo menos um n\u00famero"); ruaToggle(n,"liberar"); ruaSelected={}; ruaUpdateSelectedCount(); ruaGrid.querySelectorAll(".rua-num-btn").forEach(function(b){b.style.borderColor="transparent";b.style.boxShadow="none";}); });
            $("#rua-batch-bloquear").click(function() { var n=Object.keys(ruaSelected); if(!n.length) return alert("Selecione pelo menos um n\u00famero"); ruaToggle(n,"bloquear"); ruaSelected={}; ruaUpdateSelectedCount(); ruaGrid.querySelectorAll(".rua-num-btn").forEach(function(b){b.style.borderColor="transparent";b.style.boxShadow="none";}); });
            ruaLoadNumbers(true);
        })();
        <?php } ?>
